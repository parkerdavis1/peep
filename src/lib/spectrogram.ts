...
function updateTimeRuler(inner: HTMLElement, wrapper: HTMLElement, timeRuler: HTMLElement): void {
	const buf = appState.audioBuffer;
	if (!buf) return;
	...
	const labelCandidates = LABEL_CANDIDATES.filter((n) => n % tickInterval === 0);
	const labelInterval =
...
export function updateTimeBar(): void {
	...
	appState.timeEndText = formatTime(((scrollLeft + wrapperWidth) / totalWidth) * dur);
}
