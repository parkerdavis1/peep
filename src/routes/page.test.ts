...
describe('Application Flow', () => {
	...
	beforeEach(() => {
		...
		class AudioContextMock {
			...
			currentTime = 0;
			resume = vi.fn();
			decodeAudioData = vi.fn().mockResolvedValue(new AudioBuffer({ length: 44100, sampleRate: 44100 }));
			createBufferSource = vi.fn().mockReturnValue({ connect: vi.fn(), start: vi.fn(), stop: vi.fn(), disconnect: vi.fn() });
			createGain = vi.fn().mockReturnValue({ gain: { value: 1, setValueAtTime: vi.fn(), linearRampToValueAtTime: vi.fn() }, connect: vi.fn(), disconnect: vi.fn() });
		}
		...
		class OfflineAudioContextMock {
			...
			createGain = vi.fn().mockReturnValue({ gain: { value: 1, setValueAtTime: vi.fn(), linearRampToValueAtTime: vi.fn() }, connect: vi.fn() });
			createBiquadFilter = vi.fn().mockReturnValue({ frequency: { value: 0 }, Q: { value: 0 }, connect: vi.fn() });
			startRendering = vi.fn().mockResolvedValue(new AudioBuffer({ length: 44100, sampleRate: 44100 }));
			destination = {};
		}
	...
	test('transitions to editor and handles zoom when audio buffer is loaded', async () => {
		...
		// Load fake buffer
		appState.audioBuffer = new AudioBuffer({ length: 44100, sampleRate: 44100 });
		await tick();
		...
		const zoomInBtn = Array.from(target.querySelectorAll('.zoom-btn')).find(b => b.textContent === '+') as HTMLButtonElement;
		zoomInBtn.click();
		await tick();
		...
		const zoomOutBtn = Array.from(target.querySelectorAll('.zoom-btn')).find(b => b.textContent === '−') as HTMLButtonElement;
		zoomOutBtn.click();
		await tick();
	...
	test('handles keyboard shortcuts', async () => {
		...
		const rightArrow = new KeyboardEvent('keydown', { code: 'ArrowRight', bubbles: true });
		document.dispatchEvent(rightArrow);
		await tick();
		expect(appState.markerPos).toBeCloseTo(0.505, 3);

		const leftArrow = new KeyboardEvent('keydown', { code: 'ArrowLeft', bubbles: true });
		document.dispatchEvent(leftArrow);
		await tick();
		expect(appState.markerPos).toBeCloseTo(0.5, 3);

		// Up/Down arrows control zoom
		const upArrow = new KeyboardEvent('keydown', { code: 'ArrowUp', bubbles: true });
		document.dispatchEvent(upArrow);
		await tick();
		expect(appState.zoomLevel).toBe(2);

		const downArrow = new KeyboardEvent('keydown', { code: 'ArrowDown', bubbles: true });
